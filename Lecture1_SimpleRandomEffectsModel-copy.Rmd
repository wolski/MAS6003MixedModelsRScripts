...
$$ \textrm{Yield}~_{ij} = \mu_i +\epsilon_{ij} \mbox{ for Farm i}$$
...
```{r, eval=FALSE}
(fit2 <- lm(Yield~Farm, data=Cropdata))
model.matrix(fit2)
```
would have fit the model 

$$ \textrm{Yield}_{ij} = \begin{cases}
...
$$
\begin{aligned} 
\textrm{Yield}~_{ij} &=\alpha+\beta_i +\epsilon_{ij} \mbox{ for Farm i}\\
...
This is a case where random effects models are useful. We don't really care about the different farm means - those farms will never occur again.  We are interested in  farms not in thesample - in other words, we want to know the distribution of possible yields at different farms.
